...
public:
    ...
    struct Player {
        ...
        std::int64_t join_time_ms = 0;
        std::optional<std::int64_t> stopped_since_ms;
        bool retired = false;
    ...
    void SetDogRetirementTime(std::chrono::milliseconds value) noexcept {
        dog_retirement_time_ms_ = value.count();
    }
...
private:
    ...
    void RetirePlayer(Player& player, std::int64_t retire_time_ms);
    ...
    postgres::RecordsRepository* records_repository_ = nullptr;
    std::int64_t current_time_ms_ = 0;
    std::int64_t dog_retirement_time_ms_ = 60'000;
};
