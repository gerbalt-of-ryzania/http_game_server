...
class RequestHandler {
   public:
    ...
    void operator()(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        ...
        api::RequestContext ctx{
            infra::GenerateRequestId(),
            std::string(req.method_string()),
            path_str,
        };

        infra::log::Info("http_request_started", {
        ...
        if (path_str.starts_with(Endpoints::Api)) {
            net::post(api_strand_, [this, ctx = std::move(ctx), path_str, req = std::move(req),
                                    send = std::forward<Send>(send)]() mutable {
                ProcessApi(ctx, std::string_view{path_str}, std::move(req), std::move(send));
            });
            return;
        }

        static_files_.Handle(ctx, req, std::string_view{path_str}, std::forward<Send>(send));
    }
   ...
   private:
    ...
    template <typename Body, typename Allocator, typename Send>
    void ProcessApi(const api::RequestContext& ctx, std::string_view path,
                    http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        if (path == Endpoints::JoinGame) {
            return HandleJoinGame(ctx, std::move(req), std::forward<Send>(send));
        }
        if (path == Endpoints::Players) {
            return HandlePlayers(ctx, std::move(req), std::forward<Send>(send));
        }
        if (path == Endpoints::GameState) {
            return HandleGameState(ctx, std::move(req), std::forward<Send>(send));
        }
        if (path == Endpoints::PlayerAction) {
            return HandlePlayerAction(ctx, std::move(req), std::forward<Send>(send));
        }
        if (path == Endpoints::GameTick) {
            if (!http_tick_enabled_) {
                return send(api::responses::MakeApiError(ctx, req, http::status::bad_request, "badRequest",
                                                         "Invalid endpoint"));
            }
            return HandleGameTick(ctx, std::move(req), std::forward<Send>(send));
        }
        if (path == Endpoints::Records) {
            return HandleRecords(ctx, std::move(req), std::forward<Send>(send));
        }

        HandleMapRequest(ctx, path, std::move(req), std::forward<Send>(send));
    }

    template <typename Body, typename Allocator, typename Send>
    void HandleMapRequest(const api::RequestContext& ctx, std::string_view path,
                          http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        if (req.method() != http::verb::get && req.method() != http::verb::head) {
            return send(api::responses::MakeApiError(ctx, req, http::status::method_not_allowed, "invalidMethod",
        ...
        if (path == Endpoints::Maps) {
            ...
            if (req.method() == http::verb::head) {
                return send(api::responses::MakeApiJsonHeadResponse(ctx, req, http::status::ok, body.size()));
            }
            return send(api::responses::MakeApiJsonResponse(ctx, req, http::status::ok, body));
        }
        ...
        if (path.starts_with(Endpoints::MapsPrefix)) {
            ...
            if (id_str.empty()) {
                return send(
                    api::responses::MakeApiError(ctx, req, http::status::bad_request, "badRequest", "Bad request"));
            }
            model::Map::Id id{std::string{id_str}};
            const model::Map* map = game_.FindMap(id);
            if (!map) {
                return send(
                    api::responses::MakeApiError(ctx, req, http::status::not_found, "mapNotFound", "Map not found"));
            }
            return send(api::responses::MakeApiJsonResponse(
                ctx, req, http::status::ok, json::serialize(api::serializers::SerializeMap(*map, map_extra_data_))));
        }

        return send(
            api::responses::MakeApiError(ctx, req, http::status::bad_request, "badRequest", "Invalid endpoint"));
    }

    template <typename Body, typename Allocator, typename Send>
    void HandleJoinGame(const api::RequestContext& ctx, http::request<Body, http::basic_fields<Allocator>>&& req,
                        Send&& send) {
        ...
        if (!api::middleware::IsJsonContentType(req)) {
            return send(api::responses::MakeApiError(ctx, req, http::status::bad_request, "invalidArgument",
        ...
        try {
            ...
            return send(
                api::responses::MakeApiJsonResponse(ctx, req, http::status::ok, json::serialize(response_body)));
    ...
    template <typename Body, typename Allocator, typename Send>
    void HandlePlayers(const api::RequestContext& ctx, http::request<Body, http::basic_fields<Allocator>>&& req,
                       Send&& send) {
        if (req.method() != http::verb::get && req.method() != http::verb::head) {
            return send(api::responses::MakeApiError(ctx, req, http::status::method_not_allowed, "invalidMethod",
                                                     "Invalid method", "GET, HEAD"));
        ...
        if (!token) {
            return send(api::responses::MakeApiError(ctx, req, http::status::unauthorized, "invalidToken",
                                                     "Authorization header is missing"));
        }

        const model::Game::Player* current_player = game_.FindPlayerByToken(*token);
        if (!current_player) {
            return send(api::responses::MakeApiError(ctx, req, http::status::unauthorized, "unknownToken",
    ...
    template <typename Body, typename Allocator, typename Send>
    void HandleGameState(const api::RequestContext& ctx, http::request<Body, http::basic_fields<Allocator>>&& req,
                         Send&& send) {
        if (req.method() != http::verb::get && req.method() != http::verb::head) {
            return send(api::responses::MakeApiError(ctx, req, http::status::method_not_allowed, "invalidMethod",
                                                     "Invalid method", "GET, HEAD"));
        ...
        const model::Game::Player* current_player = AuthorizePlayer(ctx, req, "Authorization header is required", send);
        if (!current_player) {
            return;
        }
    ...
    template <typename Body, typename Allocator, typename Send>
    void HandlePlayerAction(const api::RequestContext& ctx, http::request<Body, http::basic_fields<Allocator>>&& req,
                            Send&& send) {
        if (req.method() != http::verb::post) {
            return send(api::responses::MakeApiError(ctx, req, http::status::method_not_allowed, "invalidMethod",
                                                     "Invalid method", "POST"));
        ...
        return send(api::responses::MakeApiJsonResponse(ctx, req, http::status::ok, "{}"));
    }

    template <typename Body, typename Allocator, typename Send>
    void HandleGameTick(const api::RequestContext& ctx, http::request<Body, http::basic_fields<Allocator>>&& req,
                        Send&& send) {
        if (req.method() != http::verb::post) {
            return send(api::responses::MakeApiError(ctx, req, http::status::method_not_allowed, "invalidMethod",
                                                     "Invalid method", "POST"));
    ...
    template <typename Body, typename Allocator, typename Send>
    void HandleRecords(const api::RequestContext& ctx, http::request<Body, http::basic_fields<Allocator>>&& req,
                       Send&& send) {
        if (req.method() != http::verb::get && req.method() != http::verb::head) {
            return send(api::responses::MakeApiError(ctx, req, http::status::method_not_allowed, "invalidMethod",
                                                     "Invalid method", "GET, HEAD"));
        ...
        const std::string body = json::serialize(
            api::serializers::SerializeRecords(records_repository_.GetRecords(start, max_items, ctx.request_id)));
        if (req.method() == http::verb::head) {
            return send(api::responses::MakeApiJsonHeadResponse(ctx, req, http::status::ok, body.size()));
        }
        return send(api::responses::MakeApiJsonResponse(ctx, req, http::status::ok, body));
    ...
    template <typename Body, typename Allocator, typename Send>
    const model::Game::Player* AuthorizePlayer(const api::RequestContext& ctx,
                                               const http::request<Body, http::basic_fields<Allocator>>& req,
                                               std::string_view missing_message, Send& send) {
        const auto token = api::middleware::ExtractBearerToken(req);
        if (!token) {
            send(api::responses::MakeApiError(ctx, req, http::status::unauthorized, "invalidToken", missing_message));
            return nullptr;
        }

        const model::Game::Player* player = game_.FindPlayerByToken(*token);
        if (!player) {
            send(api::responses::MakeApiError(ctx, req, http::status::unauthorized, "unknownToken",
                                              "Player token has not been found"));
            return nullptr;
        }
