...
class RequestHandler {
   ...
   private:
    ...
    void HandleRecords(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        ...
        if (!ParseRecordsQuery(req.target(), start, max_items)) {
        ...
        json::array arr;
        for (const auto& record : records_repository_.GetRecords(start, max_items)) {
            json::object obj;
    ...
    static bool ParseRecordsQuery(std::string_view target, std::size_t& start, std::size_t& max_items) {
    ...
    static bool ParseSizeT(std::string_view value, std::size_t& result) {
        if (value.empty()) {
            return false;
        }
        std::size_t parsed = 0;
